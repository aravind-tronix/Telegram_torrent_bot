...
def top_movies(chatid, context):
    ...
    for query in res:
        ...
        for div in soup.find_all(
            "div", {"class": "l30719a994ed675b3e5543484a83d6141b0edb709 clearfix"}
        ):
    ...
    context.bot.send_message(
        chat_id=chatid,
        text="Feedbacks are welcomed. Contact Admin\n @aravind_at_telegram",
        parse_mode=telegram.ParseMode.HTML,
...
def now_playing(chatid):
    ...
    for movie in movies:
        ...
        send_data = (
            json.dumps(movies_all).strip("{}").replace(",", "\n").replace('"', "")
        )
        final_data.append(send_data + "\n\n")
    final_str = listToString(final_data)
    my_bot().send_message(
        chatid,
        text="Click a movie name to copy. Paste and get torrent links."
        + "\n\n"
        + final_str,
        parse_mode=telegram.ParseMode.HTML,
    )
...
def load_more(chatid):
    ...
    url = (
        "https://api.themoviedb.org/3/movie/now_playing?api_key=cc348043650d1146104248ee9c810fa6&language=en-US&page="
        + str(search)
    )
    response1 = requests.request("GET", url)
    data1 = json.loads(response1.text)
    ...
    my_bot().send_message(
        chatid,
        text="Click a movie name to get torrent links." + "\n\n" + final_str,
        parse_mode=telegram.ParseMode.HTML,
    )
    my_bot().send_message(chatid, "show more movies?\n\n" + "/load_more")
